...
let screen_width:number = window.innerWidth;
let screen_height:number = window.innerHeight;
...
let move_touch_x:number = 0.0;//控制运动方向
let move_touch_y:number = 0.0;
let if_touch:boolean = false;
...
let zoom_touch_dist:number = 0.0;
let zoom_cam_factor:number = 1.0;
...
function handle_touchstart(event:any) {
	...
	let num_touches:number = touches.length;
    ...
    {
        if_touch = true;
        const relative_x:number = touches[0].clientX - screen_width / 2.0;
        const relative_y:number = -touches[0].clientY + screen_height / 2.0;

        const relative_len:number = Math.sqrt(relative_x * relative_x + 
...
function handle_touchmove(event:any) {
    ...
    if(num_touches > 1)
    {
        let zoom_touch_dist_new:number = Math.sqrt(Math.pow(touches[0].clientX - touches[1].clientX, 2) +
            Math.pow(touches[0].clientY - touches[1].clientY, 2));
...
function handle_touchend(event:any) {
